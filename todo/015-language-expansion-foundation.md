...
Establish one shared contract for adding Python, C#, C, C++, Rust, and a browser WebAssembly target without duplicating dispatch, artifact, toolchain, provenance, or acceptance policy. This task changes infrastructure only; it adds no language and no semantic capability.

## Current evidence and gap

[`../index.js`](../index.js), [`../src/frontends/index.js`](../src/frontends/index.js), and [`../src/backends/index.js`](../src/backends/index.js) use closed conditionals and one `SemanticLanguage` union. `supportedLanguages` implies that every listed language has both a frontend and textual backend. Generation returns one text artifact, although C#, C, Rust, and browser Wasm require project/helper/binary artifacts. Runtime specs know the original five commands directly. Rich mappings model generated text coordinates, not binary byte offsets. Task 010 is intentionally later and concerns semantic multi-file modules, so it cannot be used to hide these single-module artifact needs.
...
- Define separate, inspectable capability descriptors for frontend parsing, text generation, binary generation, artifact multiplicity, round-trip support, source-map form, and acceptance runner.
...
- Define `GeneratedArtifactSet` for one semantic module: stable language/target ID, ordered unique relative paths, `text` or `binary` content, media type, role (`entry`, `source`, `manifest`, `support`, `mapping`, or `loader`), and provenance. Reject absolute paths, traversal, duplicates, platform separators, empty content where invalid, and multiple entries.
...
Select the official Tree-sitter Node binding plus the official `tree-sitter-python`, `tree-sitter-c-sharp`, `tree-sitter-c`, `tree-sitter-cpp`, and `tree-sitter-rust` grammar packages as the parser route for Tasks 016–020. Before the first manifest change, record a reproducible qualification table covering:
...
- a small corpus per grammar proving complete child traversal and precise locations, plus an explicit check that no generated archive, Git URL/tarball dependency, postinstall download, or vendored parser binary enters the repository.
...
- Implementation must update the canonical Dockerfile and TensorBuzz environment with distribution/toolchain sources deliberately; this planning task does not choose or install those packages.
...
- Add a binary mapping form with half-open generated byte ranges, ordered/non-overlapping validation, originating semantic nodes/symbols, and synthetic-byte reasons. Never reinterpret byte offsets as UTF-16 text columns.
...
- Registry specs prove each role independently, duplicate/unknown/missing-role rejection, stable ordering, and public discovery agreement.
...
- Parser qualification is recorded and all five selected Tree-sitter routes pass it before their adapters may merge.
...
Any new language adapter/backend, semantic feature, user dependency resolution, implicit file reads, package installation, remote compiler service, plugin architecture, runtime download, parser bundling, Task 010 modules, or Wasm ABI.
