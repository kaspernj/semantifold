...
The qualified matrix is Gradle 8.13, Android Gradle Plugin 8.11.1, Kotlin plugin 2.2.10, Temurin JDK 21.0.8, compile/target API 35, minimum API 23, Build Tools 35.0.0, command-line tools 11076708, immutable emulator 35.6.11 build 13610412, and immutable `system-images;android-35;google_apis;x86_64` revision 9. Online bootstrap checksum-verifies every direct official archive and is separate from the actual offline lint/unit/assemble/dependency/APK checks. Lint remains aborting with warnings as errors and disables only the icon-free baseline's `MissingApplicationIcon` plus `OldTargetApi`, whose moving-latest recommendation conflicts with the owner-pinned API-35 matrix. The sole library exceptions are exact non-transitive `testImplementation("junit:junit:4.13.2")` and `testRuntimeOnly("org.hamcrest:hamcrest-core:1.3")`, with empty application runtime and APK leakage rejection.
