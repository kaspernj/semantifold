...
- Status: `in progress — deterministic generation/provenance implemented; materialization and Apple acceptance deferred`
...
- The manifest declares exactly which files are generator-owned. Generator artifact/resource paths use a closed portable ASCII alphabet and refuse traversal, case-fold, and file/directory-prefix collisions. Filesystem symlink/ancestor safety, publication, pre-existing-path handling, regeneration, and overwrite protection remain deferred until a separately designed native boundary can satisfy the full transaction contract.
...
The owner-selected generator/provenance scope is implemented on the Task 026 topic branch. The `ios` registry identity is application-only; it consumes Task 010 programs or normalized modules, enforces the Task 022 Tasks 001–004 Swift subset, lowers deterministic per-module captured Swift, and generates the unsigned SwiftUI/project/configuration/test/asset/manifest artifact set. Closed configuration/assets, a complete portable ASCII artifact/resource path domain, exact hashes, rich Ruby-to-Swift provenance, renderer-owned configuration spans, stable semantic program identities/linkage, deterministic Xcode IDs, byte-identical regeneration, and fail-loud generation diagnostics have focused coverage. Generated Xcode source/project syntax is deterministic but not yet platform-qualified.

Kasper selected “Defer the materializer and ship the generator/provenance scope.” No public project writer, implementation, result type, or package payload is retained. Node's public filesystem API cannot combine genuine atomic no-replace publication of a complete directory with descriptor-relative no-follow staging/publication/cleanup; ordinary rename would weaken the contract. A future implementation requires a separately authorized, reviewed, platform-qualified native boundary. Fresh packed-consumer checks repeat deterministic generation, semantic manifest identity checks, dependency listing, and strict typing after ordinary install and clean `npm ci` with empty npm configs, fresh caches, the public registry, and no inherited credentials, while proving the materializer surface is absent. Real Ruby output equals generated semantic Swift output under exact Swift 6.3.3 Linux debug and optimized builds.
...
AC07 and AC12–AC14 remain deferred: no transaction-safe project publication boundary exists, no qualified Xcode build/list/test result exists, no official named iOS Simulator has booted/installed/launched the app, and XCUIAutomation has not observed exact `semantifold-output` text. The generated Xcode format therefore remains platform-unqualified. Task 026 must not be marked delivered until those proofs and the remaining coordinator-owned CI/merge steps complete. The one independent review is already consumed. Task 027 remains dependent on Task 026; Task 028 remains a separate Android lane.
