...
Focused semantic, frontend, backend, project, provenance/mapping, and real-runtime specs cover nested `List<T>`, independent and repeated parameters, string/integer instantiations, present optional evidence, order-independent contextual validation after empty arguments, closed optional record construction/member substitution, recursive dynamic documentation, exact PHP getter receivers, explicit Java construction applications, native application syntax, missing/free/duplicate/raw/wrong-arity types, forbidden arguments on non-generic records, inference conflict/failure/recursion, bounds/defaults/variance/wildcards, casts/reflection/operators, malformed external IR, and imported generic declarations. The bounded independent-review correction pass has been implemented locally; exact-head review confirmation, TensorBuzz CI, and merge remain pending. No standard-library facade/provider behavior, pairwise rewrite, fallback type, wrapper, specialization, release, publication, deployment, review automation, or external-system mutation is included.

The package version remains `0.3.0`. Independent review, exact-head TensorBuzz CI, merge, and any later publication remain pending and coordinator-owned.
