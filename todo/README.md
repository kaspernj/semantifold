...
Phase W follows the [watch/build/check design](../docs/watch-build-pipeline.md). Tasks 038–040 and 043 now supply transactional persistent publication, the strict one-shot project CLI, Java's target-owned non-executing `javac` plan, and deterministic serialized watch orchestration. Ordinary operation still generates only; `--check` compiles Java without running it. Tasks 041–042 and 044–045 remain the check/watch delivery chain in dependency order, with Task 045 as its single terminal acceptance task. Task 046 later defines truthful watch/check policies for Wasm and application targets without reopening deferred Apple/Xcode work.
