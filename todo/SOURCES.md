...
This is a selected integration route, not approval of unpinned future dependencies. Task 015 must qualify exact releases and lockfile integrity before use: compatible Tree-sitter language ABI, Node 24 install/load behavior on the canonical lane, typed APIs, licenses, UTF-8 byte locations converted to Semantifold UTF-16 locations, comments/error/recovery nodes, complete Tasks 001–004 syntax coverage, and absence of postinstall downloads or vendored parser binaries. Dependencies normally use ordinary npm registry packages. Task022 is the documented exception: after the registry Swift package failed its packaging requirement, an explicit route amendment selected one full-SHA public fork source archive with lock integrity and no generated release asset or runtime download. A failed qualification still blocks the task rather than permitting a parser or source-text fallback.
