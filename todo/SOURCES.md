...
| [`@driftlog/tree-sitter-dart`](https://registry.npmjs.org/%40driftlog%2Ftree-sitter-dart/1.0.4) | `1.0.4` | rejected pending route amendment for Task 029: ABI 15, Node 24/CST, source/tag identity, signatures, and load pass, but clean install leaves an undeclared `@driftlog/node-addon-api` dependency-tree entry |
...
| `@driftlog/tree-sitter-dart` | `1.0.4` | `1.0.4` | [immutable source tag](https://github.com/herocodess/tree-sitter-dart/tree/v1.0.4), commit `04ef841a94a6482f17b99ff2b6ad1195aa59935a`, [qualification record](../docs/parser-qualification.md#task-029-dart-grammar-qualification--2026-09-15) |
...
Evidence note: Dart is a later strict VM/native source+target lane because Flutter supplies its main requested platform value and Dart browser integers would introduce a second numeric contract. Task 029 rejects unscoped `tree-sitter-dart@1.0.0`, whose language object is incompatible with the exact root binding, and investigated exact registry `@driftlog/tree-sitter-dart@1.0.4`. Its immutable source/tag and registry parser, scanner, declarations, and license bytes match; metadata declares ISC while the unchanged shipped license text uses the MIT-form grant, an upstream discrepancy recorded explicitly in the qualification table. Node 24, ABI 15, typed CST, recovery, coordinates, corpus, signatures, and load pass, but a clean install and the packed-consumer proof both report an extraneous `node_modules/@driftlog/node-addon-api` directory containing generated node-addon-api Makefiles. That violates the clean dependency-tree gate, so Task 029 is blocked pending an explicit parser-route amendment; Dart 3.13.3 differential execution was not reached, and no cleanup hook, fallback parser, or substitute package was added. Flutter remains a separate application backend requiring both real Android emulator and iOS Simulator jobs before claiming those platforms; it introduces no plugin/platform-channel semantics and no store-signing automation.
