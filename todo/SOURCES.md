...
- Parser adapters: [`../src/frontends/javascript-typescript.js`](../src/frontends/javascript-typescript.js), [`../src/frontends/php.js`](../src/frontends/php.js), [`../src/frontends/ruby.js`](../src/frontends/ruby.js), [`../src/frontends/java.js`](../src/frontends/java.js), and [`../src/frontends/python.js`](../src/frontends/python.js).
...
This is a selected integration route, not approval of unpinned future dependencies. Task 015 must qualify exact releases and lockfile integrity before use: compatible Tree-sitter language ABI, Node 24 install/load behavior on the canonical lane, typed APIs, licenses, UTF-8 byte locations converted to Semantifold UTF-16 locations, comments/error/recovery nodes, complete Tasks 001–004 syntax coverage, and absence of postinstall downloads or vendored parser binaries. Dependencies normally use ordinary npm registry packages. Tasks022 and 023 are documented exceptions: after the registry packages failed their packaging requirements, explicit route amendments selected full-SHA public-fork source archives with lock integrity and no generated release asset or runtime download. A failed qualification still blocks the task rather than permitting a parser or source-text fallback.
...
| [`@driftlog/tree-sitter-dart`](https://registry.npmjs.org/%40driftlog%2Ftree-sitter-dart/1.0.4) | `1.0.4` | rejected: ABI 15 and Node/CST checks pass, but clean install leaves an undeclared `@driftlog/node-addon-api` dependency-tree entry |
| [`tree-sitter-dart-orchard`](https://registry.npmjs.org/tree-sitter-dart-orchard/0.7.0) | `0.7.0` | selected by the authorized Task 029 amendment; immutable `v0.7.0`, ABI 14, Node 24/CST, clean install/`npm ci`, signatures, packed consumer, and exact Dart 3.13.3 differential pass locally |
...
| `tree-sitter-kotlin` | full-SHA HTTPS source archive | `0.4.0` at `57c35ad1a80ccd2a0ebd8fffe852f0d13a20acd0` | [fork commit](https://github.com/kaspernj/tree-sitter-kotlin/tree/57c35ad1a80ccd2a0ebd8fffe852f0d13a20acd0), tag `v0.4.0-semantifold.1`, [qualification record](../docs/parser-qualification.md#task-023-kotlinjvm-grammar-qualification---2026-09-09) |
| `tree-sitter-dart-orchard` | `0.7.0` | `0.7.0` | [immutable source tag](https://codeberg.org/grammar-orchard/tree-sitter-dart-orchard/src/tag/v0.7.0), commit `9322cd5e1266c60983ae0ff921fbb4e77e903781`, [qualification record](../docs/parser-qualification.md#task-029-dart-grammar-qualification--2026-09-15) |
...
| Java compiler/runtime | exact Ubuntu base-specific `openjdk-25-jdk-headless` builds | OpenJDK `25.0.4.1+1-1~26.04.4` development and `25.0.4+7-1~24.04` TensorBuzz | `Dockerfile`, `tensorbuzz.yml`, shared Java and exact Kotlin/JVM `java25` execution |
...
| Kotlin compiler/toolchain | exact official JetBrains compiler ZIP | Kotlin/JVM 2.4.20 on exact Ubuntu24.04 OpenJDK25.0.4+7 or Ubuntu26.04 OpenJDK25.0.4.1+1 package identities | checksum-qualified Kotlin2.4.20 archive in `Dockerfile`/`tensorbuzz.yml`, configured `SEMANTIFOLD_KOTLINC` discovery, JVM25 native qualification |
| Dart compiler/toolchain | exact official stable Linux x64 SDK ZIP | `Dart SDK version: 3.13.3 (stable) (Tue Sep 1 01:07:17 2026 -0700) on "linux_x64"` | SHA-256 `549c182cffbdc6864df7509c16fec646c73fe6cb8a18c2cb572db1292f300cd7` in `Dockerfile`/`tensorbuzz.yml`, exact `SEMANTIFOLD_DART` discovery, offline format/analyze/VM/native qualification |
...
- [.NET support policy](https://dotnet.microsoft.com/en-us/platform/support/policy), [Ubuntu .NET installation](https://learn.microsoft.com/en-us/dotnet/core/install/linux-ubuntu-install), and [C# language versioning](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/language-versioning) — .NET 10 active-LTS selection, Ubuntu `dotnet-sdk-10.0`, and matching C# 14 profile.
- [C# warning waves](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/compiler-messages/warning-waves), [warning options](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/compiler-options/errors-warnings), and [.NET globalization settings](https://learn.microsoft.com/en-us/dotnet/core/runtime-config/globalization) — warning level 10, warnings-as-errors, and invariant runtime properties used by the fixed project.

Evidence note: C# `long` and `bool` are value types, while `string` is a sealed immutable reference type; nullable reference annotations are compile-time metadata backed by flow warnings, not distinct runtime types. Task 017 selects .NET 10 LTS, C# 14, `net10.0`, nullable enabled, warning wave 10 with warnings as errors, invariant globalization, and checked signed-64-bit arithmetic. It permits no boxing, `dynamic`, overload, identity, conversion, or nullable approximation.
...
Evidence note: Dart is a strict VM/native source+target lane because browser Dart integers would introduce a second numeric contract. Task 029 rejects unscoped `tree-sitter-dart@1.0.0`, whose language object is incompatible with the exact root binding, and exact registry `@driftlog/tree-sitter-dart@1.0.4`, whose clean install produces extraneous scoped node-addon-api residue. Kasper's narrow 2026-09-15 amendment selects exact public registry `tree-sitter-dart-orchard@0.7.0`. Its annotated `v0.7.0` tag peels to npm `gitHead` `9322cd5e1266c60983ae0ff921fbb4e77e903781`; MIT provenance, pinned release-generation route, registry signature, Node 24/Tree-sitter 0.25.1 loading, ABI 14 typed CST, recovery, coordinates, corpus, clean isolated install/`npm ci`, packed-consumer behavior, and exact Dart 3.13.3 offline format/analyze/VM/native checks pass locally. No cleanup hook, fallback parser, Git/archive dependency, vendored build, or private package was added. Review, exact-head CI, merge, and post-merge verification remain pending. Flutter remains a separate application backend requiring both real Android emulator and iOS Simulator jobs before claiming those platforms; it introduces no plugin/platform-channel semantics and no store-signing automation.
...
- [WebAssembly tool conventions](https://github.com/WebAssembly/tool-conventions), especially [debugging/source maps](https://github.com/WebAssembly/tool-conventions/blob/main/Debugging.md) — a Wasm source map uses zero-based generated line 0 with the generated column equal to the zero-based module byte offset, plus a `sourceMappingURL` custom section.
...
Evidence note: core Wasm is a low-level binary instruction/module format with no ambient browser access or string type. Browser interaction is supplied through explicit imports and the JavaScript API. Task 021 is therefore target-only: an internal deterministic binary encoder, versioned imports/exports, fixed exported memory with literal data plus a checked bounded scratch arena for runtime UTF-8 concatenation, a JavaScript loader/HTML harness, byte-offset provenance, zero-based-line external source map/custom section, independent validation, and mandatory real headless-browser execution. It does not claim WAT or `.wasm` source normalization.
