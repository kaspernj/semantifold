...
Prove the standard-library portability architecture end to end with versioned canonical blocking TCP client, text-stream, output, and resource capabilities; Ruby `TCPSocket` and bounded one-string `puts` language compatibility stdlib/facades; and a PHP target host provider/native binding using genuine native stdout plus `fsockopen`, `fgets`, and `fclose`.
...
- `Output.v1_write_line(text)` outputs the string exactly once and appends LF only when the string does not already end in LF. It never removes or replaces an existing line ending, so an LF- or CRLF-terminated string is emitted unchanged and an unterminated string gains exactly one LF.
...
## Ruby language compatibility stdlib/facades

- Resolve only the documented static `require "socket"` identity, genuine Ruby `TCPSocket` symbol, and genuine unqualified `puts` or equivalent `Kernel.puts` symbol. Compile portable facades preserving the supported `TCPSocket.new(host, port)`, receiver `gets`, and `close` API shape plus exactly `puts(string)`/`Kernel.puts(string)`.
...
- The `puts` compatibility function calls `Output.v1_write_line(text)`; it does not reuse the existing target print statement. Compile both facades with the Ruby application through semantic IR. Generated PHP may retain `TCPSocket` and `puts` inside a collision-safe compatibility namespace/import alias, but their behavior calls canonical provider operations rather than PHP APIs directly.
- Reject `TCPSocket.open`, block forms, subclassing/reopening/monkey-patching, constants or methods beyond the exact profile, dynamic require/send, custom resolver hooks, socket options, timeout arguments, native extensions, and user-defined/shadowed `TCPSocket`. Also reject multiple `puts` arguments, arrays or recursive output, implicit string conversion, custom output separators, and all other Ruby `puts` behavior.

## PHP target host provider/native binding

- Implement connect with protected native `fsockopen`, line read with protected native `fgets` plus `feof`/error discrimination, and close with protected native `fclose`. Implement `Output.v1_write_line` by computing the canonical bytes once and writing them completely to native PHP stdout, without adding a second LF when the string already ends in LF. Capture/normalize PHP warnings, short writes, and failure returns at the provider boundary so generated application output and failures are deterministic.
...
- Source diagnostics distinguish unresolved/shadowed/modified `socket`, `TCPSocket`, or `puts`/`Kernel.puts`; unsupported member/argument/block/option or output form; dynamic behavior; and facade profile mismatch at exact locations.
...
- Run generated PHP with the real configured `php` CLI. The server sends an LF-terminated line, a CRLF-terminated line, and a final unterminated Unicode line before clean close. Pass each present line to the compiled `puts` facade and assert exact output bytes prove unchanged LF/CRLF termination, exactly one LF added to the final line, then EOF absence with no output.
- Exercise the condition-controlled Ruby `gets`/`puts line` loop through the compiled facades without special read-loop recognition or whole-program refactoring. Assert explicit close occurs once after EOF and the server/process terminate cleanly.
...
- Negative acceptance covers dynamic/unresolved/shadowed/monkey-patched Ruby forms, unsupported socket APIs/options, excluded multi-argument/array/coercive/custom-separator `puts` behavior, missing/version-incompatible PHP provider, namespace collision, protected native access, unused-module exclusion, and late transactional failure.
...
TCP servers/listeners, TLS, UDP, Unix sockets, nonblocking I/O, async/event loops, configurable timeouts, cancellation, concurrency, DNS policy beyond basic host resolution, proxying, arbitrary socket options, binary framing, output beyond bounded one-string `puts`, seek, arbitrary encodings, every Ruby `Socket`/`IO` API, other source facades, other target providers, or direct idiomatic lowering.

## Completion criteria

- Proved Ruby `TCPSocket` and bounded `puts` source forms compile with executable facades to PHP and link canonical v1 capabilities to the native PHP provider with no pair-specific bridge.
