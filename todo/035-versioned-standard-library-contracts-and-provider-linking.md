...
The target host provider registry (`createStdlibProviderRegistry`, `listStdlibProviders`) registers one deterministic provider per adopted target (PHP, Ruby, JavaScript, TypeScript, Java) with the distinct `provider` role, exact version/operation sets, provider dependencies, and derived protected native entry names `__semantifold_provider_<target>_<operation>`. Malformed records, duplicate identities, unknown targets, and role overstatement fail with `STDLIB_PROVIDER_INVALID`. Capability negotiation (`negotiateStdlibProviders`) computes the complete transitive requirement closure, selects one stable compatible provider set regardless of requirement order, admits provider-only dependency modules reached through selected provider dependencies into the result closure with empty operation sets, verifies that every selected provider covers every required operation of its module, and reports `STDLIB_PROVIDER_MISSING`, `STDLIB_PROVIDER_AMBIGUOUS`, `STDLIB_PROVIDER_DEPENDENCY_MISSING`, `STDLIB_PROVIDER_CYCLE`, and `STDLIB_OPERATION_UNDECLARED` before any generation.

Program generation links negotiated providers transactionally: used canonical operations are collected per module, one tree-shaken provider artifact is materialized at `providers/<target>/semantifold/task034/resource-probe.<ext>` with synthetic provenance for the artifact-materializing targets, entry modules call only the protected names, and the artifact set carries a truthful `SemantifoldStdlibLink` v1 descriptor (modules, providers, native entries, version). Module path planning rejects module identities that fold onto provider paths with `STDLIB_NAME_COLLISION`; forged or mismatched contract descriptors, missing contracts, and late capability failures discard the whole candidate with `STDLIB_LINK_FAILURE` or `UNSUPPORTED_CAPABILITY`. Repeated parse/generate cycles are byte-for-byte stable. The provider support body is byte-identical to the single-module Task 034 support carrier, per language comment dialect. PHP program entries fully qualify capability-owned nominal types because generated entries are namespaced while provider classes remain global. Java resolves the same negotiated provider record but hosts the shared support in the first operation-using module instead of a separate provider artifact: that module's generated class declares the support as public static final nested classes plus public static protected shims for the program-wide used operations, non-owner modules pull referenced capability types through nested-type imports and the shims through one static import of the owner class, and the link descriptor reports the owner module file as the provider carrier.

The protected binding stays compiler-owned: source may invoke canonical operations only under compiler authority, protected spellings are rejected in source, same-language declarations shadow ordinary spellings, and a focused collision spec proves provider native lookup uses only the protected binding. Real-toolchain execution runs the linked program with real `php`, `ruby`, `node`, `tsc` + `node`, `javac` + `java` and asserts identical deterministic probe output, including effect order, typed failures, clean EOF presence, and every-path close across the provider boundary; a separate two-module Java regression compiles and runs the shared-owner layout where the non-entry module hosts the nested support and the entry imports it; missing tools fail the lane. Language compatibility facades (Task 036) and the blocking TCP slice (Task 037) remain excluded.
