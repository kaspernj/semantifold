...
Add a strict versioned Semantifold project manifest and a real package CLI that turns an explicit source snapshot into one or more transactionally published generated targets in a one-shot `build` operation.
...
- an ordered list of targets with registered target ID, explicit role (`text`, `binary`, or `application`), owned output root, and optional separate owned compiler/build root reserved for Task 040;
...
Reject duplicate module/target identities, unsupported roles, feature-incompatible targets, output/source overlap, nested target roots, and any path that escapes or aliases another declared root.
...
- Parse/link once, generate every declared target from that semantic snapshot, stage all targets, and commit the project generation all-or-nothing through Task 038.
...
- Return exit `0` only for a committed generation. Configuration, source-read, parse, semantic, generation, or publication failure returns non-zero and retains prior outputs.
- Package the bin in the public tarball and prove a fresh credential-free packed consumer can run it.

## Tests

- Strict manifest acceptance/rejection, including unknown keys, duplicate IDs, role mismatch, path aliases/escapes, source/output overlap, and target nesting.
...
- Multiple target entries stage from one source hash and commit together; one late target failure leaves every prior target generation unchanged.
...
- A fresh consumer can declare explicit sources/targets and run one deterministic transactionally published build.
