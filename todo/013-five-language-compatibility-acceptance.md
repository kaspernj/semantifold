...
- Aggregate `npm test` was intentionally not run locally under this task's focused-only instruction; broad discovery remains TensorBuzz-owned. This local delivery adds no semantic or stdlib capability, pairwise rewrite, source-text fallback, map iteration, expanded-language acceptance, Phase 2/3 behavior, release, publication, or deployment claim.
