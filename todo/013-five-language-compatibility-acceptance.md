...
- Five substantial parser-native fixtures normalize to one complete Tasks 001–008 module. The spec-only structural comparator removes only locations, provenance, declaration IDs, and their regenerated resolved-call links without JSON cloning or public API exposure. Representative nested type constituents, map entries and children, optional operators, loop bindings, and nested call arguments retain exact parser-owned locations.
- Every one of the 25 original-five source→target routes validates, emits, reparses, and preserves the complete modeled meaning. The five source-derived artifacts for each target are asserted byte-identical before one distinct PHP, Ruby, JavaScript, TypeScript, and Java artifact executes through the real required commands with exact `compat\nbranch-ok!\npresent\nabsent\n7\n2\n3\n5\n4\n` output. The original minimal fixture remains covered separately, and no map is iterated.
- The compact rejection corpus crosses all five languages with parse, unsupported-form, missing-type, resolution, type, flow, malformed external-IR, and target-capability failures. It also protects representative Phase 0/1 exclusions, reserved names, Java integer range, PHP key rules, and transactional failure through every generation API.
- The pure acceptance file was immediately green at `3/3`; no artificial product failure was introduced. The diagnostic work found two concrete defects: dedicated PHP and Babel location regressions each proved RED at `0/1` because native syntax errors lacked the requested filename, then GREEN at `1/1` after minimal parser-coordinate normalization. The final Task 013 diagnostic corpus is `4/4`; the adjacent diagnostics file is `4/4`.
- Final focused validation is `16/16` assertions across the Task 013 acceptance/diagnostic/location files, adjacent diagnostics, public API, and packed-consumer proof. Lint, strict root/workspace typecheck, build, high-severity audit with zero vulnerabilities, production/full dependency listings, package dry-run, parsed inventory assertions, and `git diff --check` pass. The 546-entry package contains expected public/build/source documentation surfaces and exactly the four intended bundled dependencies, with specs, task-private records, nested archives, and credential-like paths absent.
- The mandatory local aggregate gate is `LANG=C.UTF-8 LC_ALL=C.UTF-8 npm test`; focused Task 013 files supplement rather than replace it, and TensorBuzz repeats broad coverage after local handoff validation. This local delivery adds no semantic or stdlib capability, pairwise rewrite, source-text fallback, map iteration, expanded-language acceptance, Phase 2/3 behavior, release, publication, or deployment claim.
