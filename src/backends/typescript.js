...
export function generateTypeScript(module, writer) {
  ...
  errors.forEach((error, errorIndex) => {
    ...
    writer.synthetic(" extends Error {}", "TypeScript unchecked error scaffolding", [error], [path])
  })
 ...
 * @param {import("./writer.js").SourceWriter} writer - Source-aware writer.
 */
function emitStdlibProviderImports(module, writer) {
...
function emitStatement(writer, statement, indent, path, canonicalizeZero) {
  ...
  if (statement.kind == "ForEachMapStatement") {
    ...
    emitExpression(writer, statement.map, `${path}/map`, "typescript", identity)
  ...
  if (statement.kind == "RaiseStatement") {
    ...
    emitExpression(writer, statement.error.message, `${path}/error/message`, "typescript", identity)
