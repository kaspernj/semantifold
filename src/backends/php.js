...
export function generatePhp(module) {
  const functions = module.functions.map((functionDeclaration) => {
    ...
    const consequent = emitExpression(branch.consequent[0].expression, (name) => `$${name}`)
    const alternate = emitExpression(branch.alternate[0].expression, (name) => `$${name}`)

    return [
      `function ${functionDeclaration.name}(${parameters}): int`,
      "{",
  ...
  const prints = module.entryPoint.body.map((statement) => {
    return `echo ${emitExpression(statement.expression, (name) => `$${name}`)}, PHP_EOL;`
  }).join("\n")
