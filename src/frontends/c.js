...
const binaryOperations = new Map([
  ["+", "PhpAdd"], ["-", "Subtract"], ["*", "Multiply"], ["==", "JavaEqual"], ["!=", "JavaNotEqual"],
...
/** Strict field-and-token consumer for the private frozen C CST. */
export class CReader {
  ...
  fail(node, detail = node.type) {
    unsupportedSyntax("c", detail, this.location(node))
  }
   ...
   */
  validateTree(node) {
    for (const {node: child} of node.children) this.validateTree(child)
    if (node.error || node.missing || node.hasError) {
      throw new SemantifoldDiagnostic({code: "PARSE_ERROR", language: "c", location: this.location(node), message: `C parser recovery at '${node.type}'.`})
    }
    ...
    if (node.type == "comment") {
      ...
      // A terminal CR belongs to ordinary CRLF; content after a bare CR is not a C line comment.
  ...
  identifier(node) {
    ...
    if (node.type != "identifier" || !isCIdentifier(name)) this.fail(node, "reserved or unsupported identifier")
    return name
  }
   ...
   */
  type(node) {
    if (!["primitive_type", "type_identifier"].includes(node.type)) this.fail(node, "scalar type shape")
    const type = sourceScalarType("c", this.text(node), this.location(node))
  ...
  call(node) {
    ...
    if (name.type != "identifier" || list.type != "argument_list") this.fail(node, "direct call shape")
  ...
  /**
   * Decodes a parser-backed ordinary C string token as UTF-8 bytes.
    ...
    for (let index = 1; index < parts.length - 1; index++) {
      ...
      if (child.type == "string_content") {
        ...
        if (simple.has(value)) bytes.push(/** @type {number} */ (simple.get(value)))
          ...
          // C consumes every adjacent hex digit; the frozen grammar caps each escape node at four.
  ...
  /**
   * Converts supported expressions without assuming C argument evaluation order.
  ...
  expression(node, nested = false) {
    ...
    if (node.type == "number_literal") {
      ...
      if (!/^(?:0|[1-9][0-9]*)$/u.test(spelling) || !Number.isSafeInteger(Number(spelling))) this.fail(node, "non-safe or noncanonical integer")
    ...
    if (node.type == "call_expression") {
      ...
      if (name == "INT64_C" || name == "SEMANTIFOLD_STRING") {
        if (call.arguments.length != 1) this.fail(node, "literal macro arity")
        if (name == "SEMANTIFOLD_STRING") return withParserRanges({kind: "StringLiteral", location, value: this.string(call.arguments[0])}, {literal: location})
  ...
  statement(node) {
    ...
    if (node.type == "declaration") {
      ...
      if (!typeNode) return missingType("c", "local", location)
  ...
  /**
   * Converts one file-local semantic function.
  ...
  function(node) {
    ...
    this.shape(node, ["storage_class_specifier", type, declarator, body])
    if (this.text(this.parts(node)[0]) != "static" || declarator.type != "function_declarator") this.fail(node, "file-local function required")
    const name = this.field(declarator, "declarator")
  ...
  /**
   * Removes only the exact main cleanup and status scaffolding.
  ...
  main(node) {
    ...
    this.shape(parameters, ["(", "parameter_declaration", ")"])
    const voidParameter = this.parts(parameters)[1]

    this.shape(voidParameter, ["primitive_type"])
    if (this.text(this.parts(voidParameter)[0]) != "void") this.fail(voidParameter, "main parameters")
    const parts = this.parts(body)
    const cleanup = parts.at(-3)
    const status = parts.at(-2)

    if (!cleanup || !status || body.type != "compound_statement") this.fail(body, "main cleanup")
    this.shape(body, ["{", ...parts.slice(1, -1), "}"])
    this.shape(cleanup, ["call_expression", ";"])
    const call = this.call(this.parts(cleanup)[0])

    if (this.text(call.name) != "semantifold_cleanup" || call.arguments.length != 0) this.fail(cleanup, "entry cleanup")
    ...
    return {kind: "EntryPoint", location: this.location(node), body: {kind: "Block", location: this.location(body),
      statements: this.generated ? this.generatedStatements(body.children.map(({node: child}) => child).filter((child) =>
        child.startIndex > body.startIndex && child.endIndex <= cleanup.startIndex))
        : parts.slice(1, -3).map((child) => this.statement(child))}}
  }
  ...
  module(root) {
    ...
    this.generated = root.children.some(({node}) => node.type == "comment" && this.text(node) == "/* semantifold:program:c:v1 */")
    if (!this.generated) this.rejectScaffoldComments(root)
    if (root.type != "translation_unit") this.fail(root, "translation unit required")
    const parts = this.parts(root)
    const definitions = parts.slice(1).filter((node) => !this.generated || node.type != "declaration")

    if (parts.length < 3 || parts[0].type != "preproc_include" || definitions.some((node) => node.type != "function_definition")) this.fail(root, "translation unit members")
    const include = parts[0]
    const header = this.field(include, "path")
    ...
    if (this.generated) {
      const canonical = generateCProgram({module: validated}).artifacts[0].content

      if (typeof canonical != "string") throw new Error("C backend returned a nontext entry artifact.")
      this.compareTree(root, parseCst(canonical).root, new CReader(this.filename, canonical))
    }
    return validated
  }
    ...
    const visit = (block, inherited) => {
      ...
      for (const statement of block.statements) {
        ...
        if (statement.kind == "PrintStatement") {
          ...
          if (!expected) throw new Error("C print statement lost its parser helper identity.")
   ...
   * @param {CReader} canonical - Canonical token reader.
   * @returns {void}
   */
  ...
  generatedStatements(children) {
    ...
    while (cursor < nodes.length) {
      ...
      const match = begin.type == "comment" && /^\/\* semantifold:ordered-expression:c:v1 begin ([0-9]{6}) ([0-9a-f]{64}) \*\/$/u.exec(this.text(begin))
      ...
      if (!end || end.type != "comment" || this.text(end) != `/* semantifold:ordered-expression:c:v1 end ${match[1]} ${match[2]} */`) this.fail(end ?? begin, "ordered marker pair")
 ...
 */
export function parseC({filename, source}) {
  if (!hasOnlyUnicodeScalars(source)) {
    const coordinates = createCoordinateIndex(source)
    const offset = source.search(/[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/u)

    throw new SemantifoldDiagnostic({code: "PARSE_ERROR", language: "c", message: "C source contains a lone UTF-16 surrogate.",
      location: {filename, start: indexedPointAt(coordinates, offset), end: indexedPointAt(coordinates, offset + 1)}})
  }
  if (source.length > maximumCSourceLength) {
    const coordinates = createCoordinateIndex(source)

    return unsupportedSyntax("c", "source exceeds the frozen C parser's 32767 UTF-16 input limit", {
  ...
  try {
    snapshot = parseCst(source)
  } catch (error) {
    return parseFailure("c", error)
  }
  const reader = new CReader(filename, source)

  try {
    return reader.module(snapshot.root)
  } catch (error) {
    if (reader.generated && error instanceof SemantifoldDiagnostic && error.language == "c" &&
      error.code != "PARSE_ERROR" && error.code != "UNSUPPORTED_SYNTAX") {
      return unsupportedSyntax("c", `invalid generated ordered scaffold: ${error.code}`, error.location ?? reader.location(snapshot.root))
    }
    throw error
  }
}
