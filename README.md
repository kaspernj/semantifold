...
Semantifold is an early language-neutral semantic code toolkit. The current source tree proves a deliberately small nine-language end-to-end baseline: parse equivalent PHP, Ruby, JavaScript with JSDoc, TypeScript, Java, strictly annotated Python, canonical C#, bounded Go, and C17 programs into one typed semantic representation, then generate and genuinely execute equivalent programs in every target language. The published package remains `semantifold@0.2.0`; Task 018 does not publish a release.
