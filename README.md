...
Semantifold is an early language-neutral semantic code toolkit. The source tree supports a deliberately small twelve-language baseline: PHP, Ruby, JavaScript with JSDoc, TypeScript, Java, strictly annotated Python, canonical C#, bounded Go, C17, C++20, Rust edition 2021, and strict Swift share one typed semantic representation. Browser WebAssembly is an additional target-only ID, not a thirteenth source language. The published baseline remains `semantifold@0.3.0`; Tasks021–022 have not been released. Swift local native acceptance and its bounded independent-review correction passed locally. Exact-head TensorBuzz CI, merge, and post-merge verification remain delivery gates.
