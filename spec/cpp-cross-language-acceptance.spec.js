...
describe("C++ original-five bidirectional native acceptance", () => {
  it("generates, reparses and executes all original targets from every CPP fixture profile", async () => {
    for (const [directory, stdout] of profiles) {
      const source = await readFile(new URL("fixtures/" + directory + "program.cpp", import.meta.url), "utf8")
      const module = parse({language: "cpp", filename: "program.cpp", source})
  ...
  it("round-trips and executes each CPP fixture at O0/O2 in ordinary and sanitizer profiles", async () => {
    for (const [directory, stdout] of profiles) {
      const source = await readFile(new URL("fixtures/" + directory + "program.cpp", import.meta.url), "utf8")
      const module = parse({language: "cpp", filename: "program.cpp", source})
      const generated = generateArtifactSet({language: "cpp", module}).artifacts[0].content

      expect(generated).toEqual(source)
      expect(meaning(parse({language: "cpp", filename: "program.cpp", source: generated}))).toEqual(meaning(module))
  ...
  it("rotates all original-five sources through generated CPP reparse and real native execution", async () => {
    for (const [index, [directory, stdout]] of profiles.entries()) {
      ...
      expect(meaning(parse({language: "cpp", filename: "program.cpp", source: content}))).toEqual(meaning(module))
      const result = await executeCpp(module)
