...
describe("generated source mappings", () => {
  it("projects Dart's rich entry mapping to Source Map v3 without treating manifests as source", async () => {
    const source = await readFile(new URL("fixtures/operators/program.dart", import.meta.url), "utf8")
    ...
    assert.equal(entry.provenance.mapping.generated.filename, "bin/program.dart")
    assert.equal(entry.provenance.sourceMap.file, "bin/program.dart")
    assert.deepEqual(entry.provenance.sourceMap.sourcesContent, [source])
    assert.ok(entry.provenance.mapping.spans.some((span) => span.mappingKind == "exact" && span.role == "operator"))
    assert.ok(set.artifacts.slice(0, 2).every(({provenance}) => provenance.kind == "synthetic"))
  })
