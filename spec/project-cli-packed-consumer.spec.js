...
describe("packed Semantifold project CLI consumer", () => {
  it("installs, typechecks, and executes the credential-free package bin after install and clean ci", {timeoutMs: 300_000}, async () => {
    ...
    try {
      ...
      await writeFile(path.join(consumerDirectory, "type-consumer.mts"), `import {ProjectBuilder, SemantifoldCli} from "semantifold"
...
`)
      ...
      for (const command of ["install", "ci"]) {
        ...
        const executed = await executeFile(path.join(consumerDirectory, "node_modules/.bin/semantifold"), ["build", "--ndjson"], {
          cwd: consumerDirectory,
          env: environment
        })
        ...
        expect(records.at(-1)).toMatchObject({exitCode: 0, project: "packed-project", state: "succeeded", terminal: true})
      }
    } finally {
