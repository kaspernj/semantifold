...
describe("C++ rich and v3 occurrence provenance", () => {
  it("maps original operators, callees and consumers while generated temporaries/control remain synthetic", async () => {
    ...
    const support = mapping.spans.find(span => entry.content.slice(span.generated.start.offset, span.generated.end.offset).includes("#include"))

    expect(support.mappingKind).toEqual("synthetic")
    assert.ok(support.origin.relatedOrigins.length > 0)
  })

  it("retains the actual CPP operator and callee token ranges when collapsing nested ordered regions", async () => {
    ...
    const cSource = generateArtifactSet({language: "cpp", module: original}).artifacts[0].content
    const module = parse({language: "cpp", filename: "input.cpp", source: cSource})
    const entry = generateArtifactSet({language: "cpp", module}).artifacts[0]
  ...
  it("assigns separate temporary and mapping identities to shared semantic expression occurrences", () => {
    ...
    expect(meaning(parse({language: "cpp", filename: "program.cpp", source: entry.content}))).toEqual(meaning(module))
  ...
  it("rebuilds stale provenance and supports legacy located modules without parser metadata", () => {
    ...
    const before = generateArtifactSet({language: "cpp", module})

    module.provenance.nodes[0].path = "/forged"
    expect(generateArtifactSet({language: "cpp", module}).artifacts[0].content).toEqual(before.artifacts[0].content)
    const legacy = JSON.parse(JSON.stringify(module, (key, nested) => ["provenance", "sourceProvenance"].includes(key) ? undefined : nested))
    const set = generateArtifactSet({language: "cpp", module: legacy})
