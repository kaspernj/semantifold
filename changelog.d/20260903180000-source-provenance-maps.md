...
- Keep provenance attached to semantic nodes across transformations, index reused nodes by occurrence path, retain every deduplicated synthetic-context origin, detach immutable artifact metadata from mutable semantic locations, canonicalize CRLF positions, bound mapping validation/lookups near linearly, and correct escaped identifiers, unmapped V3 lines, Java filenames, URL-safe external directive paths, unambiguous source-content overrides, source ownership, all-origin reverse lookup, retained rich source identities and mixed provenance, unambiguous V3 intermediates, exact-subrange anchoring, and multi-span composition.
