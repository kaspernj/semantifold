...
| 029 | Dart | exact scoped community registry grammar plus differential Dart SDK checks | blocked: `@driftlog/tree-sitter-dart@1.0.4` passes Node/CST checks but leaves an undeclared `@driftlog/node-addon-api` entry in a clean installed dependency tree |
...
Task 029 rejects the old unscoped `tree-sitter-dart@1.0.0`: its grammar object is not accepted by the repository's exact `tree-sitter@0.25.1` binding. The attempted replacement is the exact public registry package `@driftlog/tree-sitter-dart@1.0.4`. It passes the parser API and CST checks below, but fails the mandatory clean-install dependency-tree gate. Task 029 is therefore blocked pending an explicit parser-route amendment. No Git/archive dependency, copied grammar, source-text fallback, compiler-output AST, vendored build, cleanup hook, or parser substitution was introduced.

| Evidence | Qualified result |
| --- | --- |
| npm distribution | Exact `@driftlog/tree-sitter-dart@1.0.4`, registry tarball `https://registry.npmjs.org/@driftlog/tree-sitter-dart/-/tree-sitter-dart-1.0.4.tgz`, integrity `sha512-QaTUyrcXZtiOtdvfcJWH41mCQi82Ri5QLYXFsaK34gpny6ZdhCCiW+sSMMxgIe62eFtrchGfEY/nBJkm7eC+Ew==`, SHA-1 `5d4d0500af283a8fec80f1f9a370d69205e77ab0`, 17 files, 274,602 compressed bytes and 6,859,499 unpacked bytes. |
| upstream identity | [`herocodess/tree-sitter-dart`](https://github.com/herocodess/tree-sitter-dart/tree/v1.0.4), immutable tag `v1.0.4`, commit and npm `gitHead` `04ef841a94a6482f17b99ff2b6ad1195aa59935a`. Registry parser/scanner/declaration/license bytes match that tag exactly; their SHA-256 values are `9be95bed705a0c8334fc074340b8bbc5b78315e9dc51c7c89c68913a682c755d`, `07a7b7818b175e9460523e705dd88d20f7b5141bac95c593d4426e6d52284996`, `5f593f781f7cb12a3dcdcab2f9c063a77b9965f537bb01c5c05958947f5ff8fd`, and `d270cb3a4985d75033bd77d875ccebff1d66e32788a3f727891e28d76132dd46`. |
| legal and registry provenance | Package metadata declares SPDX `ISC`; the byte-identical shipped/tagged `LICENSE` contains the MIT-form permission grant. This upstream metadata/text discrepancy is retained explicitly rather than relabeling either input. Both are permissive, and `npm audit signatures --json` reported no invalid or missing signatures in the repository and fresh isolated graph. |
| clean Node 24 install/load | **Failed.** A credential-free installation with an empty home, distinct empty user/global npm configuration, fresh cache, explicit public registry, Node 24.18.1, exact `tree-sitter@0.25.1`, and exact Dart grammar loads successfully: ESM import, `new Parser()`, `setLanguage`, parse, and indexed child/field traversal all work. However, `npm ls --all --json` reports `extraneous: @driftlog/node-addon-api@ …/node_modules/@driftlog/node-addon-api`. That undeclared directory contains generated `node_addon_api.Makefile`, `node_addon_api.target.mk`, `node_addon_api_except.target.mk`, and `node_addon_api_maybe.target.mk`. The same failure occurs in Semantifold's credential-free packed-consumer installation. |
| ABI, typed API, and lifecycle | The grammar declares ABI 15, accepted by the root binding's ABI 13–15 range, and exports 351 typed node descriptions. It ships declarations, `binding.gyp`, generated parser/scanner C source, and Tree-sitter headers. Its sole install action is local `node-gyp-build`; load/install code has no URL/downloader. The published tarball's 17-file list does not contain the extraneous directory, so the residue is created during installation rather than shipped in the archive. This still violates the required clean dependency graph and blocks adoption. |
...
| official compiler differential | Exact Dart SDK 3.13.3 Linux x64 remains the selected compiler/runtime, but compiler differential execution was not reached because the earlier clean-install parser gate failed. Rebuilding the canonical image cannot qualify this parser route; an explicit route amendment must resolve the package failure first. |
...
Run `npx velocious-test spec/dart-parser-qualification.spec.js` for the retained Node/CST evidence and `npx velocious-test spec/tree-sitter-legacy-packed-consumer.spec.js` for the clean packed-consumer gate. The latter currently fails at `dependencyTree.problems` with the extraneous scoped directory above. This is a parser-route blocker, not an accepted local result; no Dart compiler or product acceptance is claimed.
