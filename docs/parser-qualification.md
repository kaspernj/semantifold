...
| 029 | Dart | exact public registry grammar plus differential Dart SDK checks | package/Node/CST/packed-consumer qualification passed for `tree-sitter-dart-orchard@0.7.0`; exact Dart 3.13.3 compiler differential remains a rebuilt-image gate |
...
| official compiler differential | Exact Dart SDK 3.13.3 Linux x64 remains the selected compiler/runtime. The checked-in compiler/VM/native differential specs are ready, but the current canonical image intentionally has no Dart executable. This final qualification dimension remains for the coordinator-owned rebuild and is not replaced with compiler output as an AST. |
...
Run `npx velocious-test spec/dart-parser-qualification.spec.js` for the retained Node/CST evidence and `npx velocious-test spec/tree-sitter-legacy-packed-consumer.spec.js` for ordinary-install plus clean-`npm ci` packed-consumer proof. Both pass before the image rebuild. The rebuilt lane must additionally pass the real Dart acceptance specs; package qualification alone does not claim compiler or product acceptance.
