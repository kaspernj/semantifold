...
The profile accepts safe canonical decimal integers, `true`/`false`, ordinary noninterpolated Unicode-scalar strings, direct identifiers and calls, integer add/subtract/multiply/negate and ordering, Boolean not/short-circuit and/or, integer/Boolean equality/inequality, and string concatenation. The adapter recognizes the qualified grammar's trailing-call additive CST shape directly, so `piece(…) + piece(…)` normalizes without source-text recovery and retains left-before-right operands. Ordinary Swift String `==`/`!=` are rejected because their canonical-equivalence semantics differ from semantic scalar-sequence equality. Source integer spelling never uses target-width `Int`, overflow operators, separators, alternate radices, floating point, or implicit conversion.
...
The registered Task 042 developer plan uses the same explicit driver mode and warnings-as-errors profile for `-typecheck`, then compiles the stable `SemantifoldGenerated` module to a relative object inside a generation-owned transient directory and links generation-owned `semantifold-swift` with the nondeterministic ELF build ID disabled. This avoids the driver's randomized temporary object path entering Swift's module hash, so repeated candidates produce identical executable bytes. The object directory, HOME, XDG cache, and TMPDIR are removed after close. The developer plan never runs the executable; debug and `-O` execution remain explicit acceptance profiles.
