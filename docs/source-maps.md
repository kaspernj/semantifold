...
Reverse lookup indexes every source range in direct, derived, and synthetic-related origins. Zero-length original locations are indexed as exact points, including valid content-backed Source Map anchors at EOF; they match only their precise offset and do not change ordinary half-open range containment. When multiple containing ranges or points associate the same span with one queried position, that generated span is returned once.
