...
- JavaScript and TypeScript use relative named ESM imports/exports. Type-only imports may name records only in semantic type positions; constructing through one is rejected, and type-only exports are explicitly outside this profile. Default, star, dynamic, side-effect-only, bare-package, attributed and CommonJS forms, re-exports, namespaces, ambient modules, export assignment, and path aliases are rejected.
