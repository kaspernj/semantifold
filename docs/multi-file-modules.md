...
## Compiler-owned standard-library modules

Tasks 036–037 extend the explicit program boundary without changing Task 010 source discovery. Parser-proved stdlib identities select immutable registry definitions; the compiler supplies only their dependency-first reachable facade source and compiles it through the ordinary frontend, semantic validation, and target backend. Caller sources cannot use a compiler-owned module ID or artifact path, forge the facade descriptor, provide capability authority to application modules, import a protected provider, or inherit authority by importing or calling a facade.

Task 037's Ruby profile recognizes only exact literal `require "socket"` and the genuine bounded `TCPSocket`/`puts` bindings. The selected application binding and the exact compiler-owned socket class are the only reference-class exception admitted in a semantic program; ordinary caller-owned Task 010 class modules remain rejected. The two generated facade support paths are `semantifold/facade/ruby/socket.php` and `semantifold/facade/ruby/output.php`. Reachable canonical requirements select distinct provider paths under `providers/php/semantifold/`: `socket-client.php`, `text-stream.php`, `output.php`, and `resource.php` as required. Provider artifacts have protected synthetic provenance; application and facade artifacts retain their separate mapped sources.

Artifact planning validates the complete source/facade/contract/provider/runtime/path/namespace/provenance/mapping graph before exposing the immutable set. Repeated generation compares every path, byte, mapping, provenance record, role, and metadata value. Missing or incompatible capabilities, forged descriptors, protected access, provider-path collisions, duplicate paths, or any late validation failure return no partial artifact set.
