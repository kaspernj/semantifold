...
- Gradle `8.14`, distribution SHA-256 `61ad310d3c7d3e5da131b76bbf22b5a4c0786e9d892dae8c1658d4b484de3caa` (shared with Task 030; advanced from the delivered 8.13 baseline because Flutter 3.47.4 requires 8.14)
...
- NDK `27.0.12077973` for the Task 030 Flutter engine build only; Task 028 native application artifacts remain NDK/JNI-free
- CMake `3.22.1` for the Task 030 Flutter engine build only
- emulator `35.6.11` build `13610412` and `system-images;android-35;google_apis;x86_64` revision `9`

API 35 and Build Tools 35.0.0 are owner-pinned acceptance boundaries rather than ambient latest-version choices. Generated lint keeps warnings as errors and aborts on findings, while disabling only `MissingApplicationIcon` for the deliberately icon-free baseline, `OldTargetApi` for the conflicting moving-latest-target recommendation, and `GradleDependency` for moving compile-SDK and Build-Tools version advice that conflicts with that pinned matrix.
...
| NDK 27.0.12077973 | `android-ndk-r27-linux.zip` | `2f17eb8bcbfdc40201c0b36e9a70826fcd2524ab7a2a235e2c71186c302da1dc` |
| CMake 3.22.1 | `cmake-3.22.1-linux.zip` | `9196644852a978012caf7a4067ba1898debf6cc204c3341562771e31080d6869` |

The Android repository archives above come from Google's official [`repository2-3.xml`](https://dl.google.com/android/repository/repository2-3.xml) and [`sys-img2-3.xml`](https://dl.google.com/android/repository/sys-img/google_apis/sys-img2-3.xml) package metadata; the emulator build and published SHA-256 come from the official [Android Emulator download archive](https://developer.android.com/studio/emulator_archive). `scripts/bootstrap-android.sh` is the only online preparation stage. It verifies immutable archives, installs them directly, registers the archived emulator through a checked-in exact local-package descriptor, verifies their embedded exact revision metadata, creates the named API-35 AVD, and warms the fixed Gradle cache with a disposable project. `scripts/accept-android.sh` generates a fresh project, creates its ephemeral debug key only inside the private acceptance root, and runs lint, JUnit, debug application assembly, and instrumentation assembly with Gradle `--offline` plus pinned UTF-8, English, UTC, heap, no-daemon, no-parallel, no-build-cache settings. Missing tools, package revisions, or cache content fail with `SEMANTIFOLD_ANDROID_INFRASTRUCTURE` rather than skipping.

The only declared Gradle library dependencies are exact non-transitive `testImplementation("junit:junit:4.13.2")`, exact non-transitive `testRuntimeOnly("org.hamcrest:hamcrest-core:1.3")`, and `androidTestImplementation(files(semantifoldKotlinStdlib))` from the checksum-pinned Kotlin compiler distribution. Hamcrest Core supplies the `org.hamcrest.SelfDescribing` linkage that JUnit's Gradle runner loads before generated test code; neither unit-test library enters application compilation or runtime. The generated unit test uses only JUnit's `@Test` annotation, compares the platform list directly, and raises a platform `AssertionError`. Kotlin's implicit stdlib dependency is disabled. Compilation reads the pinned distribution's `kotlin-stdlib.jar` directly as a compiler task input, while the explicit `androidTest` file dependency packages that same verified runtime only into the instrumentation APK so generated Kotlin instrumentation is self-contained. Acceptance requires an empty debug runtime dependency graph, excludes only Gradle's exact structural `project :app (*)` self-project entry from the unit-test report, requires exactly JUnit and Hamcrest Core among external unit-test dependencies, rejects every other project dependency form, permits no JUnit, Hamcrest, `kotlin/jvm/internal`, or `kotlin/collections` payload in the application APK, and requires `kotlin.jvm.internal.Intrinsics` in the instrumentation APK. A rejected graph reports only the parser-matched full dependency descriptions, preserving Gradle syntax without exposing the complete build log.

Real emulator acceptance runs only in TensorBuzz with `/dev/kvm:/dev/kvm`. It fails before startup unless `/dev/kvm` is usable and `emulator -accel-check` affirms acceleration, exclusively locks the complete fixed port set: emulator console `5580`, its transport `5581`, and the distinct private adb server `5038` outside the emulator port range. It never invokes `adb root` or changes KVM permissions, boots `emulator-5580` with `-accel on -wipe-data -no-snapshot`, installs both APKs, launches the app, and invokes the runner through the explicit `<applicationId>.test` test application ID. Exact UTF-8 output is passed as base64 to platform instrumentation. The test checks the native labelled view, exact text, visibility, class, accessibility importance, absent content description, and activity recreation. Task 030 reuses these same two boots for its Flutter APK after the native proof; it does not add another emulator topology. CI then stops that emulator, boots a second fresh emulator, reinstalls/relaunches, and repeats. Screenshots/UI XML are always produced; failure records logcat, adb state, emulator command line, and an exact reason, then emits one fail-loud reason header plus labeled final 16 KiB excerpts from available text diagnostics while leaving binary screenshots and full process data unprinted in their capture files.
