...
The consumer environment must assert npm's ordinary `install-links=false` default without setting it. Root `acceptDependencies` accepts the exact private package and Kotlin grammar already supplied by the bundle while retaining the local `file:` and immutable HTTPS Git installation sources. The spec runs ordinary `npm install`, full `npm ls --all`, and then clean `npm ci` from the generated consumer lock, repeating dependency, simultaneous Kotlin/Go/C/CPP/Rust runtime, frozen-data, TypeScript consumer, native Rust, and Kotlin/JVM checks. Bundled lock entries have neither links nor external resolutions; real paths must remain inside the consumer. The fresh cache must contain public dependency requests but no internal or retired adapter lookup. No consumer flags, project configuration, workspace fallback, registry release of the private name, SSH, or credentials are required.
...
The Dockerfile and TensorBuzz fetch only the official Kotlin compiler 2.4.20 ZIP, verify SHA-256 `59e9ca74c7904ef2c122b12114937673ccce68de820a663f0ed66ccf8799e0b7`, install it at `/opt/kotlinc`, and probe its exact identity on OpenJDK25. The npm tarball bundles the exact qualified grammar and the packed-consumer proof repeats grammar loading, strict public typing, Kotlin round trip, and real JAR execution after ordinary install and clean `npm ci`. Exact-head TensorBuzz CI, merge, and post-merge verification remain coordinator-owned delivery gates.
