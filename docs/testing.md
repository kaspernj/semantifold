...
The Dockerfile and TensorBuzz fetch only the official Kotlin compiler 2.4.20 ZIP, verify SHA-256 `59e9ca74c7904ef2c122b12114937673ccce68de820a663f0ed66ccf8799e0b7`, install it at `/opt/kotlinc`, and probe its exact identity on OpenJDK25 while admitting only the qualified Ubuntu 24.04 or 26.04 packaging suffix. The npm tarball leaves the exact qualified grammar as an integrity-locked full-SHA HTTPS dependency, and the packed-consumer proof repeats grammar loading, strict public typing, Kotlin round trip, and real JAR execution after ordinary install and clean `npm ci`. Exact-head TensorBuzz CI, merge, and post-merge verification remain coordinator-owned delivery gates.
