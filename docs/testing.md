...
- Rust requires the exact dependency-free edition-2021 manifest and format-4 lock, qualifies both exact Rustc and Cargo identities, binds Cargo to the discovered compiler through an exact `RUSTC` tool environment path, then runs `cargo build --offline --locked` and `cargo check --offline --locked` with isolated CARGO_HOME/HOME/temp/target state and no incremental compilation. All Cargo target/cache/lock/executable state is transient.
