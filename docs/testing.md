...
Local-binding fixtures cover initialized mutable and immutable declarations, plain assignment, function/branch/entry prefixes, exact Ruby/PHP profile metadata, and source locations in all five frontends. Focused negatives cover every declaration/assignment exclusion, declaration-before-use, unresolved and duplicate bindings, callable-name and lexical no-shadowing, immutable parameters/locals, exact type preservation, and RHS-before-target validation. Ruby coverage rejects profile-like tag misspellings, places multibyte text before later function and local carrier comments, asserts their UTF-16 locations, and reparses generated Ruby with the same offset shift. Backend shape coverage applies missing, null, primitive, and array values to every task-002 declaration/assignment field to prove caller-supplied malformed IR receives located capability diagnostics before emitter dispatch. Backend runtime coverage regenerates and reparses the same binding/mutability semantics, then executes the result through all five real toolchains with exact `yes\n` output. Target-focused coverage rejects JavaScript/TypeScript `console` and Java `args`/`System` entry captures plus JavaScript/TypeScript `console` and Ruby `puts` function captures before emission. Real `tsc` coverage demonstrates TS1215 for strict-mode `arguments`/`eval` locals, requires located backend rejection before TypeScript emission, and compiles safe near-collisions; real JavaScript, Ruby, and PHP runs preserve their legal target-specific spellings.
