...
Task 038 itself does not load a project manifest, discover sources, derive compiler arguments, start a CLI or watcher, invoke language-specific tools, delete committed generations, or implement retention. Task 039 now composes its public publisher for a strict project manifest and one-shot generation CLI; compiler/check plans, watching, and retention remain later tasks.
