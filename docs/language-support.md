...
- Ruby requires an immediately associated `# @type [String]`-style comment. Ruby locals default to mutable. An immutable declaration uses the same contiguous carrier block with a separate exact `# @semantifold-immutable` line.
