...
Status: dependency-ordered plan; Tasks 032, 033, and 034 are delivered (Task 034 through PR #44, merged as `c15e0874c054b6c462304eb29538d72ed43829d6`), Task 035 is implemented locally with independent review, TensorBuzz CI, and merge remaining coordinator-owned, and Tasks 036–037 remain planned.
...
The published `semantifold@0.3.0` package cannot express the full set of imports, receiver calls, reference classes, condition-controlled loops, optional EOF, typed failures, resource identities, ownership, and close effects required by this plan. The current source tree has since delivered Task 032's general loop/control foundation, Task 033's bounded single-module reference classes and receiver calls, and Task 034's compiler-authorized unversioned effect operations, owned-resource lifetime, explicit close, optional EOF separation, and typed host-failure normalization for that cohort. The local Task 035 implementation adds versioned canonical contracts, the target host provider registry, capability negotiation, protected provider linking, and tree-shaken provider artifacts. Facade resolution remains owned by Task 036. Encoding a socket facade before those layers exist would push language behavior into parser adapters, emitters, or handwritten source-to-target bridges.
...
4. [Task 035 — versioned standard-library contracts and provider linking](../../todo/035-versioned-standard-library-contracts-and-provider-linking.md) is implemented locally. It owns canonical capability modules, the provider registry, negotiation, protected native bindings, transactional linking, and tree-shaking; only coordinator-owned review, TensorBuzz CI, and merge remain.
...
The plan itself adds no compiler/runtime behavior or implementation authority. Tasks 032, 033, and 034 are delivered; Task 035 is implemented locally as the versioned canonical contract and provider-linking hub; Tasks 036–037 remain unimplemented. Neither this plan nor those semantic prerequisites trigger release, CI-topology changes, external task mutation, or expansion into later facade/provider tasks.
