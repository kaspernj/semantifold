...
The registered Task 042 plan accepts only the exact producer-owned dependency-free `Cargo.toml`, `Cargo.lock`, and Rust sources. It uses canonical Cargo 1.98.1 for `cargo build --offline --locked --target-dir <candidate>` followed by `cargo check --offline --locked --target-dir <candidate>`. CARGO_HOME, HOME, TMPDIR, incremental state, and the target tree are isolated below the candidate build root and removed after both children close, so no dependency, cache lock, or executable is published or run. Debug/release execution remains explicit acceptance coverage.
