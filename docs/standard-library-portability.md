...
This is an incremental compatibility profile, not a promise to translate arbitrary applications or complete standard libraries. The published `semantifold@0.3.0` release does not resolve standard-library identities or provide compatibility facades, canonical capability calls, host providers, or provider linking. The current source tree has since added bounded project imports, optionals, typed errors, and Task 032 condition-controlled loops, but still lacks the reference classes/methods, resources, effects, host APIs, facades, canonical contracts, and providers required for the first portability slice.
