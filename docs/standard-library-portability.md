...
This is an incremental compatibility profile, not a promise to translate arbitrary applications or complete standard libraries. The current release does not resolve standard-library identities or provide compatibility facades, canonical capability calls, host providers, or provider linking. It also lacks the general imports, classes and methods, loops, optional values, typed errors, resources, and host APIs required for the first portability slice.
