...
const ownedPrefix = "generated/android-app/"

/**
 * Materializes Android artifacts only inside one caller-created empty private acceptance root.
 * This is test/infrastructure support, not a public package writer.
 * @param {import("../src/semantic/types.js").GeneratedArtifactSet} artifactSet - Validated Android artifact set.
 ...
 */
export async function materializeAndroidAcceptanceProject(artifactSet, acceptanceRoot) {
  const failure = "Android acceptance materialization requires a fresh non-symlink mode-0700 directory."
  let information
  ...
  if (artifactSet.target != "android" || artifactSet.artifacts.length == 0 ||
    artifactSet.artifacts.some(({path: artifactPath}) => !artifactPath.startsWith(ownedPrefix))) {
    throw new Error("Android acceptance materialization requires one validated fixed-root Android artifact set.")
  }
  ...
  try {
    for (const artifact of artifactSet.artifacts) {
      ...
      if (written.length != expected.length ||
        ...
        throw new Error(`Checksum readback failed for Android artifact '${artifact.path}'.`)
      }
  ...
  return {projectDirectory: path.join(root, "generated", "android-app"), verifiedPaths}
}
