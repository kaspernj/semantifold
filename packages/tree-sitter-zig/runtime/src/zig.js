...
import ZigLanguage from "@tree-sitter-grammars/tree-sitter-zig"
 ...
 * }>} CstNode
 */
/** @typedef {Readonly<{schema: "semantifold.parser-cst", version: 1, language: "zig", root: CstNode}>} CstSnapshot */

/**
 * Parses the pinned Zig grammar with its isolated compatible runtime.
...
export function parseCst(source) {
  ...
  // The grammar's declaration intentionally hides its native language handle as unknown.
  parser.setLanguage(/** @type {Parser.Language} */ (/** @type {unknown} */ (ZigLanguage)))
  ...
  parser.reset()
  return Object.freeze({language: "zig", root, schema: "semantifold.parser-cst", version: 1})
}
...
function snapshotNode(node, convertIndex, lineStarts) {
  ...
  for (let index = 0; index < node.childCount; index += 1) {
    ...
    if (!child) throw new Error(`Zig Tree-sitter returned no child at index ${index}`)
...
/**
 * Normalizes either UTF-8 byte offsets or UTF-16 offsets to UTF-16.
 * @param {string} source
...
function createIndexConverter(source, rootEndIndex) {
  ...
  if (rootEndIndex != utf8Length) throw new Error(`Unsupported Zig Tree-sitter index extent ${rootEndIndex} for source length ${source.length}`)
  ...
  return (index) => {
    ...
    if (converted == null) throw new Error(`Zig Tree-sitter returned a non-boundary byte index ${index}`)
    return converted
  }
}
...
/** @param {number} index @param {readonly number[]} lineStarts @returns {CstPosition} */
