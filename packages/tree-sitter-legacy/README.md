...
This independently published ESM package isolates Tree-sitter grammars that require a legacy Node runtime from Semantifold's modern Tree-sitter dependency graph.
...
This workspace is released separately from `semantifold`. Publishing requires explicit release authorization, and Semantifold must not adopt it as a runtime dependency until the registry artifact has been published and independently verified.
